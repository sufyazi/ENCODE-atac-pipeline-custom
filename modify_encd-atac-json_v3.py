...
### Main actions ############################################################
    ...
    for n in range(1, int(sample_number) + 1):
        ...
        for rep in rep_dirs:
            ...
            # get fastq file path
            fastq_files = get_fastq_filenames(rep_path)
            ...
            # if the dataset to be processed is single-end
                ...
                if fastq_files:
                    if rep[4:] == "0":
                        fastq_keys[f"atac.fastqs_rep1_R1"] = [os.path.join(working_directory, f"sample_{n}", rep, fastq_files[0])]
                    else:
                        fastq_keys[f"atac.fastqs_rep{rep[4:]}_R1"] = [os.path.join(working_directory, f"sample_{n}", rep, fastq_files[0])]
                else:
                    print(f"No fastq.gz files found in {working_directory}/sample_{n}/{rep}")
                    continue
