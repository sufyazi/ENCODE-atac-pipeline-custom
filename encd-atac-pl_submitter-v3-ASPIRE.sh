...
for json in "${json_files[@]:$counter}"; do
    ...
    if (( sample_count < json_files_len )); then
        ...
        if [[ $counter -eq 0 && $counter -lt $MAX_JOBS ]]; then
            ...
            # Run the pipeline
            caper hpc submit /home/users/ntu/suffiazi/apps/atac-seq-pipeline/atac.wdl -i "${json}" -s "${analysis_id}" --singularity --leader-job-name "${analysis_id}_${sample_id}" --local-out-dir "${local_output_dir}" --cromwell-stdout "/home/users/ntu/suffiazi/caper_logs/cromwell.${analysis_id}_${sample_id}.out"
